          ...
          >
            ...
            >
              <div className="overflow-y-auto" style={{ width: '33.333%' }}>
                {prevData && <div className="p-4"><CategorySection key={prevData.category.id} data={prevData} categories={localData.map(d => d.category)} editMode={editMode} onChoreTab={openChore} onRefresh={refresh} onLogged={onLogged} /></div>}
              </div>
              <div className="overflow-y-auto" style={{ width: '33.333%' }}>
                {currData && <div className="p-4"><CategorySection key={currData.category.id} data={currData} categories={localData.map(d => d.category)} editMode={editMode} onChoreTab={openChore} onRefresh={refresh} onLogged={onLogged} /></div>}
              </div>
              <div className="overflow-y-auto" style={{ width: '33.333%' }}>
                {nextData && <div className="p-4"><CategorySection key={nextData.category.id} data={nextData} categories={localData.map(d => d.category)} editMode={editMode} onChoreTab={openChore} onRefresh={refresh} onLogged={onLogged} /></div>}
              </div>
            </div>
          </div>
        )}
