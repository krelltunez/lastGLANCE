...
- Signing **Team** (`DEVELOPMENT_TEAM`) pre-set with automatic signing — Xcode
  resolves the certificate/profile without you picking a team.
- Marketing version synced from `package.json` (`npm run build:ios`).
- Build number left at `1` — it doesn't need bumping (see step 5).
...
**Each build/upload:**
3. **Bump the version** in `package.json` (e.g. `1.8.4` → `1.8.5`). The build
   number (`CFBundleVersion`) stays `1`: App Store Connect only requires it to
   be unique *within a marketing version*, so a fresh version frees up build 1
   again. This is why you never touch the build number on dayGLANCE either.
4. Build + sync + open Xcode:
   ```bash
   npm run cap:ios          # build, cap sync ios, version sync, open Xcode
   ```
5. In Xcode: **Product ▸ Archive ▸ Distribute App ▸ App Store Connect ▸
   Upload.** (Team and bundle id are already set; nothing to select.)
6. Wait for processing in App Store Connect; the build then appears under
   **TestFlight**.

   > Only if you ever upload two builds of the *same* version: bump
   > `CURRENT_PROJECT_VERSION` to `2`, `3`, … in the Xcode target instead.

**Inviting testers:**
7. **Internal testing** (up to 100 App Store Connect team users): no review,
   available right after processing — the fast path.
8. **External testing** (up to 10,000, via email/public link): requires a
   one-time lightweight **Beta App Review** plus "Test Information" (beta
   description, contact). The encryption answer is already handled above.
